        ...
        <>
            <div>
                <main className="main-content  mt-0">
                    <div className="page-header align-items-start min-vh-50 pt-5 pb-11 m-3 border-radius-lg" style={{ backgroundImage: 'url("/img/illustrations/blog-portafolio-1.jpg")', backgroundPosition: 'center' }}>
                        <span className="mask bg-gradient-dark opacity-6" />
                        <div className="container">
                            <div className="row justify-content-center">
                                <div className="col-lg-5 text-center mx-auto">
                                    <h1 className="text-white mb-2 mt-5">Bienvenido!</h1>
                                    <p className="text-lead text-white">Utiliza este formulario para registrarte.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div className="card z-index-0">
                                    ...
                                    <div className="card-body">
                                        <form role="form" onSubmit={submitHandler}>
                                            <div className="mb-3">
                                                <input value={data.name} onChange={changeHandler} name='name' id='name' type="text" className="form-control" placeholder="Nombre" aria-label="Name" />
                                                {errors && (<div className='text-danger mt-1'>{errors.name}</div>)}
                                            </div>
                                            <div className="mb-3">
                                                <input value={data.username} onChange={changeHandler} name='username' id='username' type="text" className="form-control" placeholder="Nombre de usuario" aria-label="Username" />
                                                {errors && (<div className='text-danger mt-1'>{errors.username}</div>)}
                                            </div>
                                            <div className="mb-3">
                                                <input value={data.email} onChange={changeHandler} type="email" name='email' id='email' className="form-control" placeholder="Correo electrónico" aria-label="Email" />
                                                {errors && (<div className='text-danger mt-1'>{errors.email}</div>)}
                                            </div>
                                            <div className="mb-3">
                                                <input value={data.password} onChange={changeHandler} type="password" name='password' id='password' className="form-control" placeholder="Contraseña" aria-label="Password" />
                                                {errors && (<div className='text-danger mt-1'>{errors.password}</div>)}
                                            </div>
                                            <div className="text-center">
                                                <button type="submit" className="btn btn-primary w-100 my-4 mb-2">Registrarse</button>
                                            </div>
                                            <p className="text-sm mt-3 mb-0 text-center">
                                                Ya tienes una cuenta? {''}
                                                <Link href={route('login')} className="text-dark font-weight-bolder">Iniciar sesión</Link>
                                            </p>
                                        </form>
                                    </div>
                                </div>
